...
// ---------------------------------------------------------------------------
// 本地保存（下载到设备）
// 网页端可用浏览器自带「图片另存为」，但 SillyTavern 的 App（WebView）
// 没有右键菜单，无法另存图片。这里提供一个下载按钮，直接把当前显示的
// 图片以 Blob 形式触发到本机下载，浏览器与 App 均可使用。
// ---------------------------------------------------------------------------
export function guessImageExtension(url = '') {
    ...
    const match = clean.match(/\.(png|jpe?g|webp|gif|bmp|avif)(?:$|[?#])/i);
    if (match) return match[1].toLowerCase().replace('jpg', 'jpg');
    return 'png';
}
...
function triggerAnchorDownload(href, filename) {
    try {
        ...
        a.href = href;
        a.download = filename || `xiaobaix-image-${Date.now()}.png`;
        a.rel = 'noopener';
...
export async function triggerImageDownload(url, filename) {
    ...
    if (isInline) {
        return triggerAnchorDownload(url, filename);
    }
    // http(s) 地址：先抓取为 Blob 再触发下载，避免在 App / WebView 中
    // 被当作页面跳转而无法保存；失败时退回直接下载链接。
    try {
        const response = await fetch(url, { credentials: 'include', cache: 'no-store' });
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const blob = await response.blob();
        const objectUrl = URL.createObjectURL(blob);
        const ok = triggerAnchorDownload(objectUrl, filename);
        setTimeout(() => { try { URL.revokeObjectURL(objectUrl); } catch {} }, 4000);
        return ok;
    } catch {
        return triggerAnchorDownload(url, filename);
    }
...
export async function downloadCurrentImage(container) {
    if (!container) return false;
    const imgEl = container.querySelector('.xb-nd-img-wrap > img') || container.querySelector('img');
    const url = imgEl?.src || '';
    if (!url) {
        if (typeof toastr !== 'undefined') toastr.warning('没有找到可保存的图片');
        return false;
    }
    const slotId = container.dataset.slotId || 'image';
    const imgId = container.dataset.imgId || '';
    const ext = guessImageExtension(url);
    const filename = `xiaobaix_${slotId}_${imgId || Date.now()}.${ext}`;
    const ok = await triggerImageDownload(url, filename);
    if (typeof toastr !== 'undefined') {
        if (ok) toastr.success('已开始下载图片到本地');
        else toastr.warning('下载失败，可尝试长按图片保存');
    }
    return ok;
}
